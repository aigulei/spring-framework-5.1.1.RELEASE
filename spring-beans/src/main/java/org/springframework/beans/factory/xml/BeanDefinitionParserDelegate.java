...
public class BeanDefinitionParserDelegate {
	...
	public void parsePropertyElements(Element beanEle, BeanDefinition bd) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			if (isCandidateElement(node) && nodeNameEquals(node, PROPERTY_ELEMENT)) {//props标签
	...
	public void parseQualifierElements(Element beanEle, AbstractBeanDefinition bd) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			if (isCandidateElement(node) && nodeNameEquals(node, QUALIFIER_ELEMENT)) {//qualifier标签
	...
	public BeanDefinitionHolder decorateBeanDefinitionIfRequired(
		...
		//<1>遍历属性，查看是否有适用于装饰的属性:attributes
		...
		// Decorate based on custom nested elements:childNodes
